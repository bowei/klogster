...
func New(groupName, namespace, podName string, client kubernetes.Interface, store *storage.Store, h *hub.Hub) *PodStreamer {
	return &PodStreamer{
		groupName: groupName,
		namespace: namespace,
		podName:   podName,
...
func (s *PodStreamer) Run(ctx context.Context) error {
	...
	for scanner.Scan() {
		...
		s.store.Append(s.groupName, s.namespace, s.podName, line)
